...
class _PartidasModalidadeScreenState extends State<PartidasModalidadeScreen> {
  ...
  Future<void> _carregar() async {
    ...
    // 1) Equipes da modalidade (para mapear IDs -> nome)
    final equipes = await _service.listarEquipes(modalidadeId: widget.modalidade.id);
    _equipeNomeById = {
      for (final e in equipes)
        if (e.id.isNotEmpty) e.id: e.nome,
    };
    ...
    // 3) Enriquecer nomes das equipes
    ...
    // 4) Filtro "Em andamento" (o back-end valida status exato, então filtramos no app)
    ...
    // 5) Ordenação: agendadaPor/iniciadaEm
    enriched.sort((a, b) {
